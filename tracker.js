...
// ─── ANY INTERACTION WHATSOEVER PREVENTS RESET ───────────────────────────────
// Listeners on window + document ensure every hover, scroll, and tap is caught.

const activityEvents = [
    'mousedown', 'touchstart', 'touchmove', 'pointerdown', 
    'pointermove', 'mousemove', 'wheel', 'scroll', 'keydown'
];

// Apply to window (Essential for mobile scroll/swipe momentum)
activityEvents.forEach(evt => {
    window.addEventListener(evt, () => onInteraction(), { passive: true });
});

// Apply to document (Backup for hover and keyboard)
activityEvents.forEach(evt => {
    document.addEventListener(evt, () => onInteraction(), { passive: true });
});

// Redundant safety for the map viewport area
if (viewport) {
    activityEvents.forEach(evt => {
        viewport.addEventListener(evt, () => onInteraction(), { passive: true });
    });
}